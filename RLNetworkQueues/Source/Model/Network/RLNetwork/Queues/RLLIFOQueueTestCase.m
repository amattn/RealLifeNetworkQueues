...
@interface RLLIFOQueueTestCase : SenTestCase
{
...
@property (nonatomic, strong) RLLIFOQueue *queue;
@end

@implementation RLLIFOQueueTestCase
@synthesize queue = _queue;

- (void)setUp;
{
    self.queue = [[RLLIFOQueue alloc] init];
}
...
- (void)testRemoveFromQueue;
{
    ...
    [self.queue enqueueObject:@"Hi3"];
    [self.queue enqueueObject:@"Hi4"];
    [self.queue enqueueObject:@"Hi5"];
    [self.queue enqueueObject:@"Hi6"];
    [self.queue enqueueObject:[NSNumber numberWithInt:1]];
    [self.queue enqueueObject:[NSNumber numberWithInt:1]];
    [self.queue enqueueObject:[NSNull null]];
...
#define CHECK_EXPECTED_COUNT STAssertEquals([self.queue count], expectedCount, @"FAILURE- expected %d, but got %d", expectedCount, [self.queue count])
    ...
    expectedCount = 3;
    CHECK_EXPECTED_COUNT;
    [self.queue removeObjectFromQueue:@"Hi3"];
    expectedCount = 2;
    CHECK_EXPECTED_COUNT;
}
